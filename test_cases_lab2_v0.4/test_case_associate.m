...
if ~ valid_outliers
    display(sprintf('outliers computed in associate.m seem to be wrong, mse=%f',mse_err_outliers));
end
valid_psi = mse_err_psi < THRESH_VALID;
if ~ valid_psi
    display(sprintf('psi s computed in associate.m seem to be wrong, mse=%f',mse_err_psi));
end
